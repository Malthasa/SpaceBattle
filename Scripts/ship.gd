...
func _physics_process(delta: float) -> void:
	...
	# Update Battery in GUI
	if ID == 0:
		%Energy1.value = energy/10
	else:
		%Energy2.value = energy/10
...
func turnlaser(dir):
	$Laser.rotate(deg_to_rad(clamp(dir,-2,2)))
...
func shoot():
	...
	b.position = position + Vector2(0,-10).rotated($Laser.global_rotation)
